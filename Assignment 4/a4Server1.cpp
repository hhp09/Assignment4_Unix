...
int main (int argc, char **argv){
  ...
  ofstream outputfile;                    // First addition, creating ofstream variable to write results to file
  ...
  if ( (childpid = fork ()) == 0 ) {	//if it’s 0, it’s child process
    ...
    while ( (n = recv(connfd, buf, MAXLINE,0)) > 0) {                  // Concatenating results, executing commands, writing to file
      printf("My process ID : %d\n", getpid());
     	printf("%s","String received from and resent to the client:");
     	puts(buf);
      char buf2[MAXLINE];
      strcpy(buf2, buf);
      strcat(buf2, " >> Output_Results.txt");
      system(buf2);
      system(buf);
      memset(buf, 0, 255);
     	send(connfd, buf, n, 0);
